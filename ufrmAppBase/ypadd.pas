...
uses
  ...
  ExtCtrls, Grids, DBGrids, StdCtrls, Buttons, jpeg, Data.DB;
...
type
  TFm_ypadd = class(TForm)
    ...
    procedure edt_dosageExit(Sender: TObject);
  private
...
uses main, data, Factory,InterfaceMedStore,pyproc;
...
begin
  ...
  DBGrd_xm.Top:=210;
  DBGrd_xm.Left:=90;
  self.Caption:=dm.sysinfo.hospitalName+'-'+'新增药品';
end;
...
begin
  // 输入内容检查
    if CheckInput then  exit;
    if application.MessageBox('您确定数据正确并保存数据吗?', '提示信息',
    ...
    Medinfo.ddd := strtointdef(Edt_ddd.Text, 0);
    if ComBox_jjyp.Itemindex = 0 then
      Medinfo.jjyp := true
    else
      Medinfo.jjyp := false;
    Medinfo.cfhsb := strtoint(edt_cfhsb.Text);
    Medinfo.cfzxdw := Combox_cfzxdw.Text;
    if CmbBx_ifzy.Itemindex = 0 then
      Medinfo.ifzy := true
    else
      Medinfo.ifzy := false;
    if CmbBx_selfyp.Itemindex = 0 then
      Medinfo.selfyp := '1'
    else
      Medinfo.selfyp := '0';
    ...
    if CmbBx_basic.Itemindex = 0 then
      Medinfo.basic := true
    else
      Medinfo.basic := false;
    if cmb_basicProvince.Itemindex = 0 then
      Medinfo.basicProvince := true
    else
      Medinfo.basicProvince := false;
    if cbb_groupbuy.Itemindex = 0 then
      Medinfo.groupbuy := true
    else
      Medinfo.groupbuy := false;

    Medinfo.StoreType := trim(Cmbx_storetype.Text);
    Medinfo.pwh := trim(edt_pwh.Text);

    if dm.AddMed(Medinfo, dm.Fopid) then
      BBtn_canel.Click;
  end;
...
{判断单位}
 ...
 begin
   ...
   begin
     ErrorMess:='药库单位与药房单位相同，药库单位/药房单位(比例)应该等于1。是否保存？';
     goto Error1;
   end;
 end
 Else
    begin
      if Trim(Edt_hsb.text)='1' then
      begin
        ErrorMess:='药库单位与药房单位不相同，药库单位/药房单位(比例)应该不等于1。是否保存？';
        goto Error1;
      end;
    end;
 ...
 Exit;
  ...
  Error1:
  begin
    if application.messagebox(pchar(ErrorMess),'信使',mb_yesno+MB_DEFBUTTON2+MB_ICONQUESTION)=idno  then
    begin
      result:=true;
      exit;
    end;
...
begin
  ...
  begin
    ...
    Edt_pym.Text:=MakeSpellCode(AnsiString(Ed_ym.text), 0, 0);
...
procedure TFm_ypadd.edt_dosageExit(Sender: TObject);
begin
  tedit(Sender).Color:=clWhite;
  tedit(Sender).Font.Color:=clblack;
  //2024.05.27 自动生产规格
  Edt_gg.Text:=trim(edt_dosage.Text)+trim(Combox_cfzxdw.Text)+'*'+
               trim(Edt_hsb.Text)+trim(ComBox_zxdw.Text)+'/'+ trim(ComBox_jldw.Text);
end;
