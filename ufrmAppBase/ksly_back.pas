...
procedure Tfm_ksly_back.FormShow(Sender: TObject);
begin
  BBtn_bmly_db.tag:=0;
  BitBtn2.tag:=0;
  //dbgrid1.left:=87;
...
procedure Tfm_ksly_back.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //aaa dm.Q_public1.DataSource:=nil;
end;

procedure Tfm_ksly_back.Edit6Enter(Sender: TObject);
begin
  BBtn_new.Click;
  Edit6.clear;Edit16.clear;BBtn_bmly_db.enabled:=false; DBGrid_qld.visible:=false;
end;

procedure Tfm_ksly_back.Edit6KeyPress(Sender: TObject; var Key: Char);
begin
 ISDIGIT(KEY);
 if key<>#13 then exit;
 with dm.q_comm do
...
procedure Tfm_ksly_back.DBGrid_qldDblClick(Sender: TObject);
begin
...
procedure Tfm_ksly_back.DBGrid_qldKeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then DBGrid_qldDblClick(Sender);
end;

procedure Tfm_ksly_back.DBGrid_qlypCellClick(Column: TColumn);
begin
...
procedure Tfm_ksly_back.DBGrid_dbypDblClick(Sender: TObject);
begin
 edit16.text:=DBGrid_dbyp.Fields[3].asstring;
 if (DBGrid_qlyp.Fields[4].ascurrency-DBGrid_qlyp.Fields[6].ascurrency)>0 then exit
  else
   begin
       ed_sfl.text:=floattostr(DBGrid_qlyp.Fields[4].ascurrency-DBGrid_qlyp.Fields[6].ascurrency);
       try
          ed_sfl.SetFocus;
       except
...
procedure Tfm_ksly_back.DBGrid_dbypKeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then DBGrid_dbypDblClick(Sender);
end;

procedure Tfm_ksly_back.Edit16Change(Sender: TObject);
begin
if edit16.text<>'' then ed_sfl.enabled:=true else  ed_sfl.enabled:=false;
end;

procedure Tfm_ksly_back.Ed_sflKeyPress(Sender: TObject; var Key: Char);
begin
  ISDIGIT(KEY);
  if key=#13 then
  begin
...
procedure Tfm_ksly_back.Ed_sflExit(Sender: TObject);
begin
  try
...
procedure Tfm_ksly_back.BBtn_bmly_dbClick(Sender: TObject);
begin
  with dm.Q_comm do
  begin
    close;
...
procedure Tfm_ksly_back.BBtn_newClick(Sender: TObject);
begin
...
procedure Tfm_ksly_back.BBtn_quitClick(Sender: TObject);
begin
  close;
end;

procedure Tfm_ksly_back.BitBtn2Click(Sender: TObject);
begin
try
    with dm.Q_comm do
    begin
