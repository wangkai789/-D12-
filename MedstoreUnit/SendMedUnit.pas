...
begin
  ...
  //---
   ...
   cxGridDBTableView2.DataController.DataSource:=dm.DS_fybq;
   ListSickBuilding();
   ...
   Panel1.Caption:=dm.sysinfo.hospitalName+'住院发药';
   self.Caption:=dm.sysinfo.hospitalName+'-'+'住院发药';
end;
...
begin
  ...
  2:begin
   ...
   case RG_SendClass.ItemIndex of
   0,1,2:dm.InitGrid(1,'MedStore_WLock.xml',cxGridDBTableView2);
   3:begin
     if dm.Q_comm1.fieldbyname('cflbName').AsString='西药' then
       dm.InitGrid(1,'MedStore_WLock.xml',cxGridDBTableView2)
     else
       dm.InitGrid(1,'MedStore_CLock.xml',cxGridDBTableView2);
   end;
...
begin
  try
    MYMedStore:=TMedStore.Create;
    arecount:=cxGridDBTableView2.DataController.DataSource.DataSet.RecordCount;
    acfclass:=999;
    if RdGrp_yzlb.ItemIndex=2  then
      ayzlb:=999
    else
    ...
    begin
       cxGridDBTableView2.DataController.DataSource.DataSet.Close;
       btn_send.Enabled:=false;
       Exit;
    end;
  except
...
begin
    ...
    Fm_report.FPatQty:=MYMedStore.SearchPatientCount(dm.Q_public1,Fsendno);
